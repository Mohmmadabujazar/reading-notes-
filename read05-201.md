...
You can create a live code example on code.webplatform.org to add to your content. Live code can be a good addition to in-page snippets. To add code to code.webplatform.org and link to it, follow these steps.
...
The body and head tags are not needed.
In the first line of code on the CSS tab, add a CSS comment with the title of your example, such as /* Button example */. You must add a comment do regardless of whether you have CSS content or not. This is essentially the same as using title in the head of a webpage.
Click the Result tab to see your example. The title for the page tab should be the same as the title of the CSS comment.
Click the All tab, and then press CTRL + S or ⌘ + S to save the page. By saving from the All tab, users will automatically see your code executed when they land on the page.
